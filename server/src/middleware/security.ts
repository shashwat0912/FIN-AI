...
// Use environment variable for CSRF secret in production to support multi-instance deployments
// Falls back to generated secret for development
const CSRF_SECRET = process.env.CSRF_SECRET || csrfProtection.secretSync();

// Warn if using generated secret in production
if (config.NODE_ENV === 'production' && !process.env.CSRF_SECRET) {
  logger.warn('CSRF_SECRET not set in environment. For multi-instance deployments, set CSRF_SECRET environment variable.');
}
...
export const recordFailedAttempt = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!attempt) {
    ...
    attempt.count++;
    const maxAttempts = config.NODE_ENV === 'development' ? 20 : 5;
