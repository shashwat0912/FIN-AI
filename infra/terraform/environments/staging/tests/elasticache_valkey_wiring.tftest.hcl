...
run "staging_valkey_wiring" {
  ...
  assert {
    ...
    error_message = "Staging Valkey must consume only private-data subnets and the EKS security group."
  }

  assert {
    condition = (
      module.elasticache_valkey.replication_group_id == "finance-ai-staging-valkey" &&
      local.valkey_replication_group_id == "finance-ai-staging-valkey" &&
      var.valkey_engine_version == "7.2" &&
      var.valkey_node_type == "cache.t4g.micro" &&
      local.valkey_environment_config.num_cache_clusters == 1 &&
      !local.valkey_environment_config.automatic_failover_enabled &&
      !local.valkey_environment_config.multi_az_enabled &&
      local.valkey_environment_config.snapshot_retention_limit == 1 &&
      local.valkey_environment_config.final_snapshot_identifier == null &&
      local.valkey_environment_config.cloudwatch_log_retention_days == 30
    )
    error_message = "Staging Valkey must use its identifier, Valkey 7.2, one Graviton node, no HA, and short retention."
  }
}
